// OPTION B: Players Page with Links to Separate Detail Pages
// Click player card to navigate to /players/:playerId
...
const PlayerCardLink = styled(Link)`
    ...
    <PlayersContainer>
      <HeroSection>
        <HeroContent>
          ...
          <PageSubtitle>
            Meet the athletes who make up the Wellington Wolves 17U Elite squad. Click any player for their full recruiting profile.
          </PageSubtitle>
        </HeroContent>
      </HeroSection>
          ...
          <RosterGrid>
            {rosterEnhanced.map((player, index) => (
              player.firstName === 'TBD' ? (
                <PlayerCard key={player.id || player.number} $index={index}>
                ...
                <PlayerCardLink key={player.id} to={`/players/${player.id}`}>
